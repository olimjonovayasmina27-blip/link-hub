...
const clickWebhookHandler = async (req, res) => {
    try {
        ...
        const generatedSign = crypto_1.default.createHash('md5').update(strToHash).digest('hex');
        ...
        if (parseInt(action) === 1) {
            ...
            if (!dbOrder)
                return res.json({ error: -5, error_note: "Order does not exist" });
...
exports.clickWebhookHandler = clickWebhookHandler;
